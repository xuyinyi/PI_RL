...
def collate_fraggraphs_pre(samples):
    # origin_graphs, motif_graphs: list of graphs:
    origin_graphs, frag_graphs, motif_graphs, smiles = map(list, zip(*samples))

    batched_origin_graph = dgl.batch(origin_graphs)
    batched_frag_graph = dgl.batch(frag_graphs)
    batched_motif_graph = dgl.batch(motif_graphs)
    return batched_origin_graph, batched_frag_graph, batched_motif_graph, smiles
