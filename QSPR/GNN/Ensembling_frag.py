...
def main():
    ...
    early_stopping = EarlyStopping(patience=params['earlystopping_patience'],
                                   path='checkpoint_ensemble_' + params['Dataset'] + '_FraGAT' + '.pt')
...
if __name__ == "__main__":
    ...
    decay_l = [0, 0, 0, 0]
    depth_l = [1, 1, 5, 4]
    dropout_l = [0, 0.20, 0.25, 0.40]
    hidden_dim_l = [51, 229, 242, 79]
    init_lr_l = [-3, -3, -3, -3]
    layers_l = [5, 5, 1, 4]
    lr_reduce_l = [0.65, 0.90, 0.90, 0.90]
    ...
    for i in range(3, 4):
        ...
        for j in range(100):
            ...
            train_loader = DataLoader(train_set, collate_fn=collate_fraggraphs, batch_size=len(train_set),
                                      shuffle=False, num_workers=0, worker_init_fn=np.random.seed(2023))
