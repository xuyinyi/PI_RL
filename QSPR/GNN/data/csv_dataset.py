...
from QSPR.GNN.utils.mol2graph import graph_2_frag, create_channels
...
class MoleculeCSVDataset(object):
    def __init__(self, df, smiles_2_graph, atom_featurizer, bond_featurizer, mol_featurizer, cache_file_path,
                 load=False, log_every=100, error_log=None, fragmentation=None):
        self.df = df
        self.cache_file_path = cache_file_path
        self._prepare(smiles_2_graph, atom_featurizer, bond_featurizer, mol_featurizer, load, log_every, error_log)
        self.whe_frag = False
    ...
    def _prepare(self, smiles_2_graph, atom_featurizer, bond_featurizer, mol_featurizer, load, log_every, error_log):
        '''
        :param
        '''
        if os.path.exists(self.cache_file_path) and load:
            ...
            save_graphs(self.cache_file_path, self.origin_graphs,
                        labels={'values': self.values, 'valid_idx': valid_idx})
    ...
    def _prepare_frag(self, fragmentation, load, log_every, error_log):
        ...
        if os.path.exists(_frag_cache_file_path) and os.path.exists(_motif_cache_file_path) and os.path.exists(
            ...
            save_graphs(_frag_cache_file_path, unbatched_frag_graphs,
                        labels={'values': self.values, 'valid_idx': valid_idx, 'frag_graph_idx': frag_graph_idx})
    ...
    def _prepare_frag_backup(self, fragmentation, load, log_every, error_log):
        ...
        if os.path.exists(_frag_cache_file_path) and os.path.exists(_motif_cache_file_path) and load:
            ...
            self.motif_graphs, motif_label_dict = load_graphs(_motif_cache_file_path)
            self.frag_graph_idx = frag_label_dict[
                'frag_graph_idx'].detach().numpy().tolist()  # frag_graph_idx, tensor[], index of graph which the single fragment belongs to
        else:
            print('Preparing fragmentation ...')
            self.frag_graphs = []
            self.motif_graphs = []
    ...
    def __getitem__(self, index):
        ...
        if self.whe_frag:
            ...
            return self.origin_graphs[index], self.values[index], self.smiles[index]
