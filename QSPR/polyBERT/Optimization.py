...
def main(params, net_params):
    ...
    early_stopping = EarlyStopping(patience=params['earlystopping_patience'],
                                   path='checkpoint_' + params['Dataset'] + '_polyBERT' + '.pt')
...
if __name__ == '__main__':
    ...
    for i in range(3, 4):
        ...
        save_file_path = os.path.join(file_path,
                                      f'{params["Dataset"]}_polyBERT_{split_seed}_{time.strftime("%Y-%m-%d-%H-%M")}.csv')
        ...
        hpbounds = {'active_function': (0, 1.99), 'num_hidden': (1, 5.99), 'hidden_dim': (32, 256.99),
