# DAPiGen Stage-0 Unified Environment v2.3 candidate

本目录保存 Stage-0 的配置、测试、验收脚本和设计记录；实际实现已安装在仓库根目录的 `RL_PPO/envs/`，兼容入口为 `RL_PPO/moldr/env_stage0.py`。v2.3 在 v2.2 上将正式 mask 改为 `closure_exact_cached`：先做 label compatibility，仅在动作将消耗最后两个 attachment 时执行定向 legacy BRICS 闭合检查。同一 100 状态开发审计中，改进 mask 两侧均为 0 假阳性/0 假阴性，且比独立全量 exact 快 129.5 倍；1,000 步 replay/restore 也已通过。但远程 staging 无 clean Git 验收坐标，因此它仍是 P1 候选，不是已验收环境。
