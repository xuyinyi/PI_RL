...
def test_frozen_test_matrix_has_unique_complete_ids():
    payload = json.loads(
        (ROOT / "reproduction/p2/configs/p2_contract_test_matrix_v1.json").read_text()
    )
    ...
    assert {row["level"] for row in payload["tests"]} == {
        "contract_validator",
        "integrated_pending",
    }


def test_three_methods_share_everything_outside_credit_seam():
    contracts = [_method_contract(PPO), _method_contract(POLICY_CC), _method_contract(MCC_PPO)]
    validate_method_contracts(contracts)
    with pytest.raises(ContractViolation, match="outside the credit seam"):
        validate_method_contracts(
            [contracts[0], replace(contracts[1], task_contract_id="changed"), contracts[2]]
        )


def test_ppo_credit_is_exact_gae_and_never_queries_or_stages_labels():
    rollout = _rollout()
    ...
    with pytest.raises(ContractViolation, match="must equal engine GAE"):
        validate_credit_estimate(
            request, replace(estimate, actor_advantages=np.asarray([1.0, -0.5]))
        )


def test_credit_estimate_fails_on_model_leak_budget_or_source_violation():
    rollout = _rollout()
    ...
    validate_credit_estimate(request, estimate)
    with pytest.raises(ContractViolation, match="changed before"):
        validate_credit_estimate(request, replace(estimate, credit_model_version_after=4))
    with pytest.raises(ContractViolation, match="exceeded"):
        validate_credit_estimate(replace(request, reserved_query_requested_calls=1), estimate)
    wrong_source = replace(
        estimate,
        evaluator_delta=replace(ledger, requested_by_source={EVALUATION: 2}),
    )
    with pytest.raises(ContractViolation, match="unauthorized evaluator source"):
        validate_credit_estimate(request, wrong_source)


def test_update_receipt_binds_actor_credit_and_critic_environment_returns():
    rollout = _rollout()
    estimate = CreditEstimate(
        method=POLICY_CC,
        source_batch_id="batch",
    ...
    with pytest.raises(ContractViolation, match="Critic targets"):
        validate_update_receipt(
            rollout, estimate, replace(receipt, critic_returns_sha256="changed")
        )


def test_pending_labels_commit_only_after_policy_advance():
    ...
    with pytest.raises(ContractViolation, match="optimizer completion"):
        validate_pending_label_commit(
            pending, replace(receipt, optimizer_step_completed=False)
        )
    with pytest.raises(ContractViolation, match="source iteration"):
        validate_pending_label_commit(pending, replace(receipt, policy_version_after=4))
